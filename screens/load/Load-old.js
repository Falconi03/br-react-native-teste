...
const Load = ({ navigation }) => {
    ...
    const [eachSku, setEachSku] = useState(false)
    ...
    useEffect(() => {
        ...
        /* produtos.map((produto, id) => {
            if (produtosSku.find((e) => e === produto.codigo)) {
                null
            } else {
                produtosSku.push(produto.codigo)

            }
        })
        setAllSku(produtosSku) */
            ...
            axios.get(`https://app.brms.com.br/api/v1/produto/lista_produto_2/?id=${produtoId}`, config)
                .then(async (res) => {
                    try {
                        ...
                        if (id + 1 === allId.length) { setEachProduto(true) }
                    } catch (error) {
    ...
    /* useEffect(() => {
        allSku.map(async (produtoSku, id) => {
            const img = `../../assets/produto/${produtoSku}-1.jpg`
            try {
                await AsyncStorage.setItem(`@br-img-loja-${String(produtoSku)}`, JSON.stringify(img))
                if (id + 1 === allSku.length) { setEachProduto(true) }
            } catch (error) {
                console.log(`NÃO FOI POSSIVEL BAIXAR O SKU ${produtoSku}`, error)
            }
        })
    }, [allSku]) */

    if (produto/*  && estoque */ && eachProduto && carrinho /* && eachSku */) {
